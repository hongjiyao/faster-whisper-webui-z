...
# Defaults for Silero 设置语音阈值为 0.3，即只有当音频信号的能量高于这个阈值时，才会将其视为语音
SPEECH_TRESHOLD = 0.4

# Minimum size of segments to process 设置处理的最小时间段为 1 秒。
MIN_SEGMENT_DURATION = 1

#设置提示窗口的最大时间为 0 秒。这是一个处理旧时间段内文本的时间窗口，以便在下一个时间段内使用。如果设置为 0，则禁用此功能。
MAX_PROMPT_WINDOW = 0 # seconds (0 = disabled)
PROMPT_NO_SPEECH_PROB = 0.2 # 设置一个阈值，当某个时间段的无声概率高于这个阈值时，将不会将该时间段的文本传递到下一个时间段。

VAD_MAX_PROCESSING_CHUNK = 60 * 60 * 3# 设置音频处理的最大块大小为 60 分钟。这是一个处理音频的时间限制

class TranscriptionConfig(ABC):
    #非语音策略、段落填充、最大无声周期、最大合并大小、最大提示窗口和初始段落索引
...
class AbstractTranscription(ABC):#采样率参数
    def __init__(self, sampling_rate: int = 16000):
        self.sampling_rate = sampling_rate

    def get_audio_segment(self, str, start_time: str = None, duration: str = None):#加载音频片段
    ...
    def get_transcribe_timestamps(self, audio: str, config: TranscriptionConfig, start_time: float, end_time: float):
        """
        获取此 VAD 方法应转录的段的开始和结束时间戳。
        参数
        音频：str
        音频文件。
        配置：TranscriptionConfig
        转录配置。
        返回
        一个开始和结束时间戳的列表，以分数秒为单位。
        """
        return 
